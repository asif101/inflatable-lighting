...
let channel = Array(300).fill(0)
let colorArray = Array(300).fill(0)
...
io.on('connection', socket => {
    ...
    socket.on('joinUnityRoom', (data, callback) => {
        // console.log((io.sockets.adapter.rooms['unityRoom']?.length ?? 0) < 1)
        // if((io.sockets.adapter.rooms['unityRoom']?.length ?? 0) < 1) {
        ...
        // }
        // else console.warn('room limit reached')
    })
})
